...
ultimo_ticket = ""
ultimo_cierre = None 
penultimo_cierre = None   
SPOTIFY_CLIENT_ID = '4da920d7fcc04fbeacdec0b9c87f4eeb'
SPOTIFY_CLIENT_SECRET = '513ac322888549348b398f29acd5eb80'

# ================= CLASES =================
class Producto:
    def __init__(self, nombre, cantidad, precio, categoria="General", proveedor="", minimo=5, fecha_actualizacion=None, costo=0, fecha_vencimiento=None):
        self.nombre = nombre
...
# ...existing code...
class Sucursal:
    def __init__(self, nombre, direccion, empleado_asignado="", productos=None, ventas=None):
        self.nombre = nombre
        self.direccion = direccion
        self.empleado_asignado = empleado_asignado
        self.productos = productos if productos is not None else {}  # {nombre_producto: cantidad}
        self.ventas = ventas if ventas is not None else []  # lista de ventas en la sucursal

    def total_ganancia(self):
        return sum(v.total for v in self.ventas)
class Venta:
    def __init__(self, productos, total, cliente="Consumidor Final", metodo_pago="Efectivo", monto_recibido=0, vendedor=None, fecha=None):
        self.productos = productos
        self.total = total
        self.fecha = fecha or datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        self.cliente = cliente
        self.metodo_pago = metodo_pago
        self.vendedor = vendedor if vendedor is not None else (usuario_actual.username if usuario_actual else "Anónimo")
...
def guardar_cliente_bd(cliente):
    ...
    if conexion:
        try:
            ...
            cursor.execute(
                "INSERT INTO clientes (nombre, nit, direccion, telefono, email) VALUES (%s, %s, %s, %s, %s)",
                (cliente.nombre, cliente.nit, cliente.direccion, cliente.telefono, cliente.email)
            )
            conexion.commit()
        except Error as e:
...
# ...existing code...
def guardar_usuario_bd(usuario):
    conexion = conectar_bd()
    if conexion:
        try:
            cursor = conexion.cursor()
            cursor.execute(
                "INSERT INTO usuarios (username, password, rol, proveedor_asociado, sucursal_asignada) VALUES (%s, %s, %s, %s, %s)",
                (usuario.username, usuario.password, usuario.rol, usuario.proveedor_asociado, usuario.sucursal_asignada)
            )
            conexion.commit()
        except Error as e:
...
def guardar_datos():
    ...
    with open('datos_sistema.json', 'w') as f:
        json.dump(datos, f, indent=4)
# Cargar último cierre al iniciar
        ...
        with open('datos_sistema.json', 'r') as f:
            ...
            for nombre, attrs in datos['inventario'].items():
                inventario[nombre] = Producto(
                    ...
                    fecha_vencimiento=attrs.get('fecha_vencimiento')  # <-- AQUÍ ESTÁ BIEN
               ) 
            ...
            # Asigna vendedor y fecha si existen
                ventas[-1].vendedor = venta_data.get('vendedor', "Anónimo")
                ventas[-1].fecha = venta_data.get('fecha', datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
...
def cargar_ultimo_cierre():
    global ultimo_cierre, penultimo_cierre
    try:
        with open("ultimo_cierre.txt", "r") as f:
            lineas = f.readlines()
            if len(lineas) >= 2:
                penultimo_cierre = datetime.strptime(lineas[0].strip(), "%Y-%m-%d %H:%M:%S")
                ultimo_cierre = datetime.strptime(lineas[1].strip(), "%Y-%m-%d %H:%M:%S")
            elif len(lineas) == 1:
                penultimo_cierre = None
                ultimo_cierre = datetime.strptime(lineas[0].strip(), "%Y-%m-%d %H:%M:%S")
            else:
                penultimo_cierre = None
                ultimo_cierre = None
    except:
...
def cerrar_dia():
    ...
    if ultimo_cierre is None:
        ventas_corte = [v for v in ventas if datetime.strptime(v.fecha, "%Y-%m-%d %H:%M:%S") <= ahora]
    else:
        ventas_corte = [v for v in ventas if ultimo_cierre < datetime.strptime(v.fecha, "%Y-%m-%d %H:%M:%S") <= ahora]
    total_corte = sum(v.total for v in ventas_corte)

    # Guardar el cierre en el historial
    guardar_historial_cierres(ahora.strftime("%Y-%m-%d %H:%M:%S"), total_corte)

    messagebox.showinfo(
        "Cierre de Día",
        f"Ventas desde el último cierre: {len(ventas_corte)}\n"
        f"Total: Bs {total_corte:.2f}\n"
        f"Último cierre: {ultimo_cierre.strftime('%Y-%m-%d %H:%M:%S') if ultimo_cierre else 'Nunca'}"
    )
    penultimo_cierre = ultimo_cierre
    ultimo_cierre = ahora
    with open("ultimo_cierre.txt", "w") as f:
        if penultimo_cierre:
            f.write(penultimo_cierre.strftime("%Y-%m-%d %H:%M:%S") + "\n")
        f.write(ultimo_cierre.strftime("%Y-%m-%d %H:%M:%S"))

    guardar_datos()  # <-- Esto asegura que todo se guarde al cerrar el día
def mostrar_historial_cierres():
    if not os.path.exists("historial_cierres.json"):
        messagebox.showinfo("Historial de Cierres", "No hay cierres registrados.")
        return
    with open("historial_cierres.json", "r") as f:
        historial = json.load(f)
    texto = "Historial de Cierres:\n\n"
    for cierre in historial:
        texto += f"Fecha: {cierre['fecha']} - Total: Bs {cierre['total']:.2f}\n"
    messagebox.showinfo("Historial de Cierres", texto)
# ================= INTERFAZ DE USUARIO =================
def mostrar_login():
...
def agregar_producto():
    ...
    fecha_vencimiento = entry_vencimiento.get().strip() or None
    try:
    ...
    if nombre in inventario:
        ...
        if respuesta:
            inventario[nombre] = Producto(nombre, cantidad, precio, categoria, proveedor, minimo, costo=costo, fecha_vencimiento=fecha_vencimiento)
            actualizar_lista()
            limpiar_campos()
    else:
        inventario[nombre] = Producto(nombre, cantidad, precio, categoria, proveedor, minimo, costo=costo, fecha_vencimiento=fecha_vencimiento)
...
def modificar_producto():
    ...
    if seleccionado:
        ...
        ttk.Label(ventana_edicion, text="Fecha de vencimiento (YYYY-MM-DD):").pack()
        entry_vencimiento_edit = ttk.Entry(ventana_edicion, font=("Arial", 12))
        entry_vencimiento_edit.insert(0, producto.fecha_vencimiento if producto.fecha_vencimiento else "")
        ...
        def guardar_cambios():
            try:
                ...
                nueva_fecha_vencimiento = entry_vencimiento_edit.get().strip() or None
...
def actualizar_lista():
    ...
    for producto in sorted(productos_mostrar, key=lambda x: x.nombre):
        alerta = ""
        tags = ()
        # Vencimiento próximo (prioridad)
        if producto.fecha_vencimiento:
            try:
                fecha_venc = datetime.strptime(producto.fecha_vencimiento, "%Y-%m-%d")
                if fecha_venc <= datetime.now() + timedelta(days=7):
                    alerta += " ⏰"
                    tags = ('vencimiento',)
            except:
                pass
        # Stock bajo (solo si no está por vencer)
        if producto.cantidad < producto.minimo and 'vencimiento' not in tags:
            alerta = "⚠️"
            tags = ('bajo',)
    ...
    lista_inventario.tag_configure('bajo', background="#e42c2c")
    # Resaltar productos próximos a vencer (amarillo claro)
    lista_inventario.tag_configure('vencimiento', background="#fff59d")
    for item in lista_inventario.get_children():
        ...
        # Si está por vencer y no tiene stock bajo, resalta por vencimiento
        elif valores and inventario[valores[0]].fecha_vencimiento:
            try:
                fecha_venc = datetime.strptime(inventario[valores[0]].fecha_vencimiento, "%Y-%m-%d")
                if fecha_venc <= datetime.now() + timedelta(days=7):
                    lista_inventario.item(item, tags=('vencimiento',))
            except:
                pass
...
def guardar_historial_cierres(fecha, total):
    try:
        historial = []
        if os.path.exists("historial_cierres.json"):
            with open("historial_cierres.json", "r") as f:
                historial = json.load(f)
        historial.append({"fecha": fecha, "total": total})
        with open("historial_cierres.json", "w") as f:
            json.dump(historial, f, indent=4)
    except Exception as e:
        print(f"Error al guardar historial de cierres: {e}")
...
def finalizar_venta():
    ...
    btn_confirmar.bind("<Return>", lambda e: confirmar_y_guardar())
    ventana_confirmacion.bind("<Return>", lambda e: confirmar_y_guardar())
...
def gestionar_clientes():
    ...
    def agregar_cliente():
        ...
        def guardar_cliente():
            ...
            guardar_cliente_bd(clientes[nit])
...
def gestionar_proveedores():
    ...
    def eliminar_proveedor():
        ...
        if not seleccionado:
            ...
            return
        nombre = lista_proveedores.item(seleccionado, 'values')[0]
    # Verificar si hay productos asociados a este proveedor
        productos_asociados = [p for p in inventario.values() if p.proveedor == nombre]
        if productos_asociados:
            messagebox.showerror("Error", f"No se puede eliminar: Hay {len(productos_asociados)} productos asociados a este proveedor.")
            return
        respuesta = messagebox.askyesno("Confirmar", f"¿Está seguro de eliminar el proveedor '{nombre}'?")
        if respuesta:
    ...
    def agregar_proveedor():
        ...
        ventana_agregar.geometry("1300x900")
        ventana_agregar.resizable(False, False)

        ttk.Label(ventana_agregar, text="Nombre de la empresa:").pack(pady=5)
        entry_nombre = ttk.Entry(ventana_agregar, font=("Arial", 12))
        entry_nombre.pack(pady=5)
        ...
        # --- Productos asociados ---
        ttk.Label(ventana_agregar, text="Productos que provee:").pack(pady=5)
        frame_productos = ttk.Frame(ventana_agregar)
        frame_productos.pack(pady=5)
        productos_proveedor = []
        # ...dentro de agregar_proveedor()...
        lista_productos = ttk.Treeview(
            ventana_agregar,
            columns=("Producto", "Cantidad", "Compra", "Venta", "GananciaUnidad", "GananciaPaquete"),
            show="headings",
            height=5
        )
        lista_productos.heading("Producto", text="Producto")
        lista_productos.heading("Cantidad", text="Cantidad")
        lista_productos.heading("Compra", text="Compra")
        lista_productos.heading("Venta", text="Venta")
        lista_productos.heading("GananciaUnidad", text="Ganancia/U")
        lista_productos.heading("GananciaPaquete", text="Ganancia/Paq")
        lista_productos.pack(pady=5, fill=tk.X, padx=10)
    

        def agregar_producto_a_lista():
            nombre = entry_producto.get().strip().title()
            cantidad = entry_cantidad.get().strip()
            precio_compra = entry_precio_compra.get().strip()
            precio_venta = entry_precio_venta.get().strip()
            if not nombre or not cantidad or not precio_compra or not precio_venta:
                messagebox.showwarning("Advertencia", "Completa nombre, cantidad y precios.")
                return
            try:
                ...
                messagebox.showwarning("Advertencia", "Cantidad y precios inválidos.")
                return
            ganancia_unidad = precio_venta - precio_compra
            ganancia_paquete = ganancia_unidad * cantidad
            ...
            lista_productos.insert("", "end", values=(
                nombre,
                cantidad,
                f"Bs {precio_compra:.2f}",
                f"Bs {precio_venta:.2f}",
                f"Bs {ganancia_unidad:.2f}",
                f"Bs {ganancia_paquete:.2f}"
            ))
...
# Entradas para producto, cantidad, precio compra y venta
        entry_producto = ttk.Entry(frame_productos, width=18)
        entry_producto.grid(row=0, column=0, padx=2)
        entry_cantidad = ttk.Entry(frame_productos, width=8)
        entry_cantidad.grid(row=0, column=1, padx=2)
        entry_precio_compra = ttk.Entry(frame_productos, width=10)
        entry_precio_compra.grid(row=0, column=2, padx=2)
        entry_precio_venta = ttk.Entry(frame_productos, width=10)
        entry_precio_venta.grid(row=0, column=3, padx=2)
        ttk.Button(frame_productos, text="Agregar", command=agregar_producto_a_lista).grid(row=0, column=4, padx=2)

        ttk.Label(frame_productos, text="Producto").grid(row=1, column=0)
        ttk.Label(frame_productos, text="Cantidad").grid(row=1, column=1)
        ttk.Label(frame_productos, text="Compra").grid(row=1, column=2)
        ttk.Label(frame_productos, text="Venta").grid(row=1, column=3)
        ttk.Label(frame_productos, text="Ganancia").grid(row=1, column=4)
        ...
        # Guardar proveedor
            proveedores[nombre] = Proveedor(
                ...
                contacto,
                telefono,
                [p["nombre"] for p in productos_proveedor],
                plazo
            )

            # Agregar productos al inventario principal
            for p in productos_proveedor:
                if p["nombre"] in inventario:
                    # Si ya existe, sumar la cantidad
                    inventario[p["nombre"]].cantidad += p["cantidad"]
                    inventario[p["nombre"]].precio = p["precio_venta"]
                    inventario[p["nombre"]].costo = p["precio_compra"]
                    inventario[p["nombre"]].proveedor = nombre
                else:
                    inventario[p["nombre"]] = Producto(
                    nombre=p["nombre"],
                    cantidad=p["cantidad"],
                    ...
                    fecha_vencimiento=p.get("fecha_vencimiento")  # <-- agrega esto
                )

            actualizar_lista_proveedores()
            actualizar_lista()  # <-- Esto actualiza la página principal y ventas
            ventana_agregar.destroy()
            messagebox.showinfo("Éxito", "Proveedor y productos agregados correctamente.\n"
                                        "Ganancia esperada por producto mostrada en la tabla.")

        btn_guardar = ttk.Button(ventana_agregar, text="Guardar", command=guardar_proveedor, style='Accion.TButton')
        btn_guardar.pack(pady=10)
        

    btn_agregar_proveedor = ttk.Button(frame_botones, text="Agregar Proveedor", command=agregar_proveedor, style='Accion.TButton')
    btn_agregar_proveedor.pack(side=tk.LEFT, padx=5)
    btn_eliminar_proveedor = ttk.Button(frame_botones, text="Eliminar Proveedor", command=eliminar_proveedor, style='Peligro.TButton')
...
def gestionar_usuarios():
    ...
    def agregar_usuario():
        ...
        def guardar_usuario():
            ...
            guardar_usuario_bd(usuarios[username])
...
def gestionar_sucursales():
    ...
    def asignar_productos():
        ...
        sucursal = sucursales[nombre_suc]

        win = tk.Toplevel(ventana_sucursales)
        win.title(f"Asignar productos a {nombre_suc}")
        win.geometry("400x400")

        productos_disponibles = list(inventario.keys())
        cantidades = {}

        row = 0
        for prod in productos_disponibles:
            ttk.Label(win, text=prod).grid(row=row, column=0, sticky=tk.W)
            cantidad = tk.IntVar(value=sucursal.productos.get(prod, 0))
            ttk.Entry(win, textvariable=cantidad, width=8).grid(row=row, column=1)
            cantidades[prod] = cantidad
            row += 1

        def guardar():
            for prod, var in cantidades.items():
                cant = var.get()
                if cant > 0:
                    sucursal.productos[prod] = cant
                elif prod in sucursal.productos:
                    del sucursal.productos[prod]
            actualizar_lista()
            win.destroy()
...
def mostrar_historial_ventas():
    ...
    def aplicar_filtros():
        ...
        for venta in sorted(ventas_filtradas, key=lambda x: x.fecha, reverse=True):
            productos_vendidos = ", ".join(list(venta.productos.keys())[:3])
            if len(venta.productos) > 3:
                productos_vendidos += "..."
    ...
    frame_lista = ttk.Frame(frame_principal)
    frame_lista.pack(fill=tk.BOTH, expand=True)
    lista_historial = ttk.Treeview(frame_lista, columns=("Fecha", "Cliente", "Total", "Método", "Vendedor", "Productos"), show="headings")
    lista_historial.heading("Productos", text="Productos")
    lista_historial.column("Productos", width=250)
    ...
    lista_historial.bind("<Double-1>", lambda e: ver_detalle_venta())
    ...
    if usuario_actual and usuario_actual.rol == "admin":
        btn_vaciar = ttk.Button(frame_botones, text="Vaciar Historial", command=vaciar_historial_ventas, style='Peligro.TButton')
        btn_vaciar.pack(side=tk.LEFT, padx=5)
    
    # Aplicar filtros iniciales
    aplicar_filtros()
def vaciar_historial_ventas():
    if usuario_actual and usuario_actual.rol == "admin":
        respuesta = messagebox.askyesno("Confirmar", "¿Seguro que deseas vaciar TODO el historial de ventas? Esta acción no se puede deshacer.")
        if respuesta:
            ventas.clear()
            guardar_datos()
            messagebox.showinfo("Éxito", "Historial de ventas vaciado correctamente.")
    else:
        messagebox.showerror("Permiso denegado", "Solo el administrador puede vaciar el historial de ventas.")
...
def generar_reporte_ventas():
    ...
    # Opciones de fecha
    ttk.Label(ventana_reporte, text="Desde (AÑO-MES-DIA):").pack()
    entry_desde = ttk.Entry(ventana_reporte, font=("Arial", 12))
    entry_desde.pack(pady=5)
    
    ttk.Label(ventana_reporte, text="Hasta (AÑO-MES-DIA):").pack()
    ...
    def generar():
        ...
        if fecha_desde or fecha_hasta:
            try:
                ...
                messagebox.showerror("Error", "Formato de fecha incorrecto. Use AÑO-MES-DIA.")
                return
...
def mostrar_dashboard():
    ...
    # Primera fila: productos y stock bajo
    frame_estadisticas1 = ttk.Frame(frame_dashboard)
    frame_estadisticas1.pack(fill=tk.X, pady=5)
    total_productos = len(inventario)
    ttk.Label(frame_estadisticas1, text=f"📦 Productos en inventario: {total_productos}", font=("Arial", 11, "bold")).pack(side=tk.LEFT, padx=20)
    productos_bajo_stock = [p for p in inventario.values() if p.cantidad < p.minimo]
    ttk.Label(frame_estadisticas1, text=f"⚠️ Stock bajo: {len(productos_bajo_stock)}", font=("Arial", 11, "bold"), foreground=COLOR_ALERTA if len(productos_bajo_stock) > 0 else COLOR_TEXTO).pack(side=tk.LEFT, padx=20)
    

    # Segunda fila: ventas hoy, último cierre
    # Segunda fila: ventas desde el último cierre, último cierre
    frame_estadisticas2 = ttk.Frame(frame_dashboard)
    frame_estadisticas2.pack(fill=tk.X, pady=5)
    ahora = datetime.now()
    if ultimo_cierre:
        ventas_hoy = [v for v in ventas if datetime.strptime(v.fecha, "%Y-%m-%d %H:%M:%S") > ultimo_cierre]
    else:
        ventas_hoy = [v for v in ventas if datetime.strptime(v.fecha, "%Y-%m-%d %H:%M:%S") <= ahora]
    ttk.Label(frame_estadisticas2, text=f"🛒 Ventas desde último cierre: {len(ventas_hoy)}", font=("Arial", 11, "bold")).pack(side=tk.LEFT, padx=20)
    if ultimo_cierre:
        ttk.Label(frame_estadisticas2, text=f"🕒 Último cierre: {ultimo_cierre.strftime('%Y-%m-%d %H:%M:%S')}", font=("Arial", 11)).pack(side=tk.LEFT, padx=20)
    else:
        ttk.Label(frame_estadisticas2, text="🕒 Último cierre: Nunca", font=("Arial", 11)).pack(side=tk.LEFT, padx=20)

    # Tercera fila: ventas e ingresos del turno actual y anterior
    frame_estadisticas3 = ttk.Frame(frame_dashboard)
    frame_estadisticas3.pack(fill=tk.X, pady=5)
    ahora = datetime.now()
    hoy_str = ahora.strftime("%Y-%m-%d")
    if ultimo_cierre:
        ventas_turno = [
            v for v in ventas
            if datetime.strptime(v.fecha, "%Y-%m-%d %H:%M:%S") > ultimo_cierre
            and v.fecha.startswith(hoy_str)
        ]
    else:
        ventas_turno = [
            v for v in ventas
            if v.fecha.startswith(hoy_str)
        ]
    ttk.Label(frame_estadisticas3, text=f"🛒 Ventas turno: {len(ventas_turno)}", font=("Arial", 11, "bold")).pack(side=tk.LEFT, padx=20)
    ingresos_turno = sum(v.total for v in ventas_turno)
    ttk.Label(frame_estadisticas3, text=f"💰 Ingresos turno: Bs {ingresos_turno:.2f}", font=("Arial", 11, "bold")).pack(side=tk.LEFT, padx=20)
    if penultimo_cierre and ultimo_cierre:
        ventas_anterior = [v for v in ventas if penultimo_cierre < datetime.strptime(v.fecha, "%Y-%m-%d %H:%M:%S") <= ultimo_cierre]
        ingresos_anterior = sum(v.total for v in ventas_anterior)
        ttk.Label(frame_estadisticas3, text=f"⬅️ Ingresos turno anterior: Bs {ingresos_anterior:.2f}", font=("Arial", 11)).pack(side=tk.LEFT, padx=20)
    ...
    ax2.set_title("Top 5 productos más vendidos", fontsize=10)
    ax2.set_xlabel("Cantidad vendida", fontsize=9)
    ax2.tick_params(axis='y', labelsize=8)  # Tamaño de letra para nombres de productos
    ax2.tick_params(axis='x', labelsize=8)  # Tamaño de letra para valores del eje x
    ...
    if usuario_actual and usuario_actual.rol == "admin":
        global btn_wilson
        btn_wilson = ttk.Button(frame_estadisticas2, text="Activar Wilson", command=toggle_wilson, style='Accion.TButton')
        btn_wilson.pack(side=tk.RIGHT, padx=10)
...
def buscar_y_reproducir_spotify(cancion):
    try:
        sp = spotipy.Spotify(client_credentials_manager=SpotifyClientCredentials(
            SPOTIFY_CLIENT_ID, SPOTIFY_CLIENT_SECRET))
        result = sp.search(q=cancion, type='track', limit=1)
        ...
        if tracks:
            ...
            return f"Reproduciendo {tracks[0]['name']} en Spotify."
        else:
            return "No encontré la canción en Spotify."
    except Exception as e:
        return f"Ocurrió un error con Spotify: {e}"
...
def procesar_comando(comando):
    ...
    elif "spotify" in comando or "pon en spotify" in comando or "reproduce en spotify" in comando:
    # Extraer el nombre de la canción después de "spotify"
        cancion = ""
        if "pon en spotify" in comando:
            cancion = comando.split("pon en spotify")[-1].strip()
        elif "reproduce en spotify" in comando:
            cancion = comando.split("reproduce en spotify")[-1].strip()
        elif "spotify" in comando:
            partes = comando.split("spotify")
            if len(partes) > 1:
                cancion = partes[1].strip()
        if cancion:
            respuesta = buscar_y_reproducir_spotify(cancion)
        else:
            respuesta = "¿Qué canción o artista quieres escuchar en Spotify?"
            hablar(respuesta)
        # Esperar respuesta del usuario
            ...
            with mic as source:
                audio = recognizer.listen(source, timeout=5)
            try:
                cancion = recognizer.recognize_google(audio, language="es-ES")
                respuesta = buscar_y_reproducir_spotify(cancion)
            except Exception:
                respuesta = "No pude entender el nombre de la canción o artista."
    ...
    elif "producto menos vendido" in comando or "menos vendido" in comando:
        ventas_por_producto = {}
        for venta in ventas:
            for producto, datos in venta.productos.items():
    ...
    elif "producto no se vende" in comando or "no se vende" in comando:
        vendidos = set()
        for venta in ventas:
            vendidos.update(venta.productos.keys())
        no_vendidos = [p for p in inventario if p not in vendidos]
        if no_vendidos:
            respuesta = "Los productos que no se han vendido son: " + ", ".join(no_vendidos)
        else:
            respuesta = "Todos los productos han tenido al menos una venta."

    elif "producto bajo" in comando or "stock bajo" in comando or "bajo stock" in comando:
        bajos = [p.nombre for p in inventario.values() if p.cantidad < p.minimo]
        if bajos:
            respuesta = "Productos con stock bajo: " + ", ".join(bajos)
        else:
            respuesta = "No hay productos con stock bajo."

    elif "cuánto tengo de" in comando or "cuanto tengo de" in comando:
        encontrado = False
        for nombre in inventario:
            if nombre.lower() in comando:
                respuesta = f"Tienes {inventario[nombre].cantidad} unidades de {nombre}."
                encontrado = True
                break
        if not encontrado:
            respuesta = "No encontré ese producto en el inventario."

    elif "productos sin ventas" in comando or "no tienen ventas" in comando or "Wilson que productos no tiene ventas" in comando:
        vendidos = set()
        for venta in ventas:
            vendidos.update(venta.productos.keys())
        no_vendidos = [p for p in inventario if p not in vendidos]
        if no_vendidos:
            respuesta = "Los productos sin ventas son: " + ", ".join(no_vendidos)
        else:
            respuesta = "Todos los productos han tenido ventas."
    ...
    elif ("mi música favorita" in comando or "mi musica favorita" in comando or
          "quiero escuchar" in comando or "busca la canción" in comando or "busca la cancion" in comando or
          "pon la playlist" in comando or "pon el video" in comando):

        if "mi música favorita" in comando or "mi musica favorita" in comando:
            cancion = "mix música favorita"
        elif "quiero escuchar" in comando:
            cancion = comando.split("quiero escuchar")[-1].strip()
        elif "busca la canción" in comando or "busca la cancion" in comando:
            cancion = comando.split("busca la canción")[-1].strip() if "busca la canción" in comando else comando.split("busca la cancion")[-1].strip()
        elif "pon la playlist" in comando:
            cancion = comando.split("pon la playlist")[-1].strip()
        elif "pon el video" in comando:
            cancion = comando.split("pon el video")[-1].strip()
        else:
            cancion = ""
        ...
        if cancion:
            ...
            webbrowser.open(url)
            respuesta = f"Buscando {cancion} en YouTube."
        else:
            respuesta = "¿Qué música o video quieres buscar?"
    ...
    elif "ventas" in comando and "de hoy" in comando:
        hoy = datetime.now().strftime("%Y-%m-%d")
...
menu_archivo.add_command(label="Cerrar Día", command=cerrar_dia)
...
ttk.Label(marco_entradas, text="Fecha de vencimiento (YYYY-MM-DD):").grid(row=4, column=0, sticky=tk.W, pady=2)
entry_vencimiento = ttk.Entry(marco_entradas, width=15, font=("Arial", 10))
entry_vencimiento.grid(row=4, column=1, sticky=tk.W, pady=2)
...
# Permitir navegación con teclado en la lista de inventario
def modificar_producto_con_enter(event):
    modificar_producto()

lista_inventario.bind("<Return>", modificar_producto_con_enter)
lista_inventario.bind("<Double-1>", lambda e: modificar_producto())  # Doble click también modifica

# Opcional: enfocar la lista al entrar a la pestaña de inventario
def enfocar_lista_inventario(event=None):
    lista_inventario.focus_set()
    # Selecciona el primer producto si hay
    items = lista_inventario.get_children()
    if items:
        lista_inventario.selection_set(items[0])

notebook.bind("<<NotebookTabChanged>>", lambda e: enfocar_lista_inventario() if notebook.index(notebook.select()) == 0 else None)
...
# Vincular evento de escritura para búsqueda automática
entry_busqueda_ventas.bind("<KeyRelease>", lambda event: buscar_producto_ventas())
...
def pasar_foco_a_lista(event):
    lista_inventario_ventas.focus_set()
    items = lista_inventario_ventas.get_children()
    if items:
        lista_inventario_ventas.selection_set(items[0])
        lista_inventario_ventas.focus(items[0])

entry_busqueda_ventas.bind("<Down>", pasar_foco_a_lista)
...
# Permitir navegación con teclado y agregar con Enter en ventas
def agregar_al_carrito_con_enter(event):
    agregar_al_carrito()

lista_inventario_ventas.bind("<Return>", agregar_al_carrito_con_enter)
lista_inventario_ventas.bind("<Double-1>", lambda e: agregar_al_carrito())  # Doble click también agrega

# Opcional: enfocar la lista al entrar a la pestaña de ventas
def enfocar_lista_ventas(event=None):
    lista_inventario_ventas.focus_set()
    items = lista_inventario_ventas.get_children()
    if items:
        lista_inventario_ventas.selection_set(items[0])

notebook.bind("<<NotebookTabChanged>>", lambda e: enfocar_lista_ventas() if notebook.index(notebook.select()) == 1 else None)
# Botones de ventas
...
btn_finalizar_venta = ttk.Button(marco_acciones_venta, text="Finalizar Venta", style='Accion.TButton',
          command=finalizar_venta)
btn_finalizar_venta.pack(side=tk.LEFT, padx=5)
btn_finalizar_venta.bind("<Return>", lambda e: finalizar_venta())
btn_eliminar_carrito = ttk.Button(marco_botones_carrito, text="Eliminar del Carrito", 
          command=eliminar_del_carrito, style='Peligro.TButton')
btn_eliminar_carrito.pack(side=tk.LEFT, padx=5)
btn_eliminar_carrito.bind("<Return>", lambda e: eliminar_del_carrito())
